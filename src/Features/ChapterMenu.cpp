#include "ChapterMenu.hpp"

#include <vector>

#include "Features/Stats/Stats.hpp"

#include "Modules/Client.hpp"
#include "Modules/Console.hpp"

#include "Utils/SDK.hpp"

#include "Game.hpp"

ChapterMenu* chapterMenu;

ChapterMenu::ChapterMenu()
    : original()
{
}
ChapterMenu::~ChapterMenu()
{
    auto chapter = client->g_ChapterContextNames;
    for (auto& original : this->original) {
        *chapter = original;
        ++chapter;
    }
    original.clear();
}
void ChapterMenu::LoadMaps(SourceGameVersion version)
{
    auto maps = std::vector<ChapterContextData_t>();

    switch (version) {
    case SourceGame_Portal2:
        break;
    case SourceGame_PortalStoriesMel:
        maps = {
            { "st_a1_tramride", 1, 1 },
            { "st_a1_mel_intro", 1, 1 },
            { "st_a1_lift", 1, 1 },
            { "st_a1_garden", 1, 1 },
            { "st_a2_garden_de", 2, 2 },
            { "st_a2_underbounce", 2, 2 },
            { "st_a2_once_upon", 2, 2 },
            { "st_a2_past_power", 2, 2 },
            { "st_a2_ramp", 2, 2 },
            { "st_a2_firestorm", 2, 2 },
            { "st_a3_junkyard", 3, 2 },
            { "st_a3_concepts", 3, 3 },
            { "st_a3_paint_fling", 3, 3 },
            { "st_a3_faith_plate", 3, 3 },
            { "st_a3_transition", 3, 3 },
            { "st_a4_overgrown", 4, 4 },
            { "st_a4_tb_over_goo", 4, 4 },
            { "st_a4_two_of_a_kind", 4, 4 },
            { "st_a4_destroyed", 4, 4 },
            { "st_a4_factory", 4, 4 },
            { "st_a4_core_access", 5, 5 },
            { "st_a4_finale", 5, 5 },
            { "sp_a1_tramride", 6, 6 },
            { "sp_a1_mel_intro", 6, 6 },
            { "sp_a1_lift", 6, 6 },
            { "sp_a1_garden", 6, 6 },
            { "sp_a2_garden_de", 7, 7 },
            { "sp_a2_underbounce", 7, 7 },
            { "sp_a2_once_upon", 7, 7 },
            { "sp_a2_past_power", 7, 7 },
            { "sp_a2_ramp", 7, 7 },
            { "sp_a2_firestorm", 7, 7 },
            { "sp_a3_junkyard", 8, 8 },
            { "sp_a3_concepts", 8, 8 },
            { "sp_a3_paint_fling", 8, 8 },
            { "sp_a3_faith_plate", 8, 8 },
            { "sp_a3_transition", 8, 8 },
            { "sp_a4_overgrown", 9, 9 },
            { "sp_a4_tb_over_goo", 9, 9 },
            { "sp_a4_two_of_a_kind", 9, 9 },
            { "sp_a4_destroyed", 9, 9 },
            { "sp_a4_factory", 9, 9 },
            { "sp_a4_core_access", 10, 10 },
            { "sp_a4_finale", 10, 10 },
        };
        break;
    case SourceGame_ApertureTag:
        maps = {
            { "gg_intro_wakeup", 1, 1 },
            { "gg_blue_only", 1, 1 },
            { "gg_blue_only_2", 1, 1 },
            { "gg_blue_only_3", 1, 1 },
            { "gg_blue_only_2_pt2", 1, 1 },
            { "gg_a1_intro4", 1, 1 },
            { "gg_blue_upplatform", 1, 1 },
            { "gg_red_only", 2, 2 },
            { "gg_red_surf", 2, 2 },
            { "gg_all_intro", 2, 2 },
            { "gg_all_rotating_wall", 2, 2 },
            { "gg_all_fizzler", 2, 2 },
            { "gg_all_intro_2", 2, 2 },
            { "gg_all_puzzle2", 3, 3 },
            { "gg_a2_column_blocker", 3, 3 },
            { "gg_all2_puzzle1", 3, 3 },
            { "gg_all_puzzle1", 3, 3 },
            { "gg_all2_escape", 3, 3 },
            { "gg_stage_reveal", 4, 4 },
            { "gg_stage_bridgebounce_2", 4, 4 },
            { "gg_stage_redfirst", 4, 4 },
            { "gg_stage_laserrelay", 4, 4 },
            { "gg_stage_beamscotty", 4, 4 },
            { "gg_stage_bridgebounce", 4, 4 },
            { "gg_stage_roofbounce", 4, 4 },
            { "gg_stage_pickbounce", 4, 4 },
            { "gg_stage_theend", 4, 4 },
            { "gg_tag_remix", 5, 5 },
            { "gg_trailer_map", 5, 5 },
        };
        break;
    case SourceGame_ThinkingWithTimeMachine:
        maps = {
            { "tm_intro_01", 1, 1 },
            { "tm_training_01b", 1, 1 },
            { "tm_map_01b", 1, 1 },
            { "tm_scene_map-update2", 1, 1 },
            { "tm_map_02b", 1, 1 },
            { "tm_map_03b", 1, 1 },
            { "tm_map_04a", 1, 1 },
            { "tm_map_05a-update2", 1, 1 },
            { "tm_map_06a", 1, 1 },
            { "tm_map_07", 1, 1 },
            { "tm_map_08", 1, 1 },
            { "tm_map_final", 1, 1 },
        };
        break;
    case SourceGame_PortalReloaded:
        maps = {
            { "sp_a1_pr_map_002", 1, 1 },
            { "sp_a1_pr_map_003", 1, 1 },
            { "sp_a1_pr_map_004", 1, 1 },
            { "sp_a1_pr_map_005", 1, 1 },
            { "sp_a1_pr_map_006", 1, 1 },
            { "sp_a1_pr_map_007", 1, 1 },
            { "sp_a1_pr_map_008", 1, 1 },
            { "sp_a1_pr_map_009", 1, 1 },
            { "sp_a1_pr_map_010", 1, 1 },
            { "sp_a1_pr_map_011", 1, 1 },
            { "sp_a1_pr_map_012", 1, 1 },
            { "mp_coop_pr_cubes", 1, 1 },
            { "mp_coop_pr_portals", 1, 1 },
            { "mp_coop_pr_teamwork", 1, 1 },
            { "mp_coop_pr_fling", 1, 1 },
            { "mp_coop_pr_loop", 1, 1 },
            { "mp_coop_pr_catapult", 1, 1 },
            { "mp_coop_pr_laser", 1, 1 },
            { "mp_coop_pr_bridge", 1, 1 },
            { "mp_coop_pr_tbeam", 1, 1 },
            { "mp_coop_pr_bts", 1, 1 },
        };
        break;
    default:
        break;
    }

    this->original.clear();

    auto chapter = client->g_ChapterContextNames;
    for (auto& map : maps) {
        this->original.push_back(*chapter);
        *chapter = map;
        ++chapter;
    }

    console->DevMsg("Loaded %i maps!\n", maps.size());
}
