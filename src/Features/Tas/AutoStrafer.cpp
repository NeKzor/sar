...
float AutoStrafer::GetStrafeAngle(const StrafeState* strafe, void* pPlayer, const CMoveData* pMove)
{
    ...
    if (strafe->type == StrafingType::Turning && !grounded) {
        ...
        cosTheta = cosf(velocity.Length2D() * velocity.Length2D() - powf(playerFriction * tau * M * A, 2) - lambda.Length2D() * lambda.Length2D()) / (2 * playerFriction * tau * M * A * lambda.Length2D());
    } else {
